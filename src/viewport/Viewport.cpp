...
Viewport::Viewport(int width, int height, float simMinX, float simMaxX, float simMinY, float simMaxY)
    : m_Width(width), m_Height(height), 
      m_SimMinX(simMinX), m_SimMaxX(simMaxX), 
      m_SimMinY(simMinY), m_SimMaxY(simMaxY) {}
...
glm::vec2 Viewport::windowToSim(const glm::vec2& windowPos) const {
    ...
    float y = m_SimMinY + (1.0f - windowPos.y / m_Height) * (m_SimMaxY - m_SimMinY); // y axis is reversed
    return glm::vec2(x, y);
}

glm::vec2 Viewport::simToWindow(const glm::vec2& simPos) const {
    float x = (simPos.x - m_SimMinX) / (m_SimMaxX - m_SimMinX) * m_Width;
    float y = (1.0f - (simPos.y - m_SimMinY) / (m_SimMaxY - m_SimMinY)) * m_Height; // y axis is reversed
    return glm::vec2(x, y);
}